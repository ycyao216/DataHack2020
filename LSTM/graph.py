...
class Graph():
    def __init__(self):
        ...
        #LSTM_layer = tf.keras.layers.LSTM(4)
        #self.pre_fc = LSTM_layer(self.x_input)

        self.flat_x = tf.reshape(self.x_input, [-1,30])

        self.W1 = tf.get_variable("FC_weights1", shape=[30,5], dtype=tf.dtypes.float32,
        ...
        self.pre_middle = tf.matmul(self.flat_x, self.W1)
        ...
        self.W2 = tf.get_variable("FC_weights2", shape=[5,1], dtype=tf.dtypes.float32,
                trainable=True, initializer=initializer1)
